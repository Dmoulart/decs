...
class EventTarget {
  ...
  dispatchEvent(event: any) {
    ...
    const list = (this as any)[EVENTS].get(event.type);
    if (list == null) return;
  ...
  addEventListener(type: any, fn: Function) {
    let events = this[EVENTS].get(type);
    if (!events) this[EVENTS].set(type, (events = []));
    events.push(fn);
  }
  removeEventListener(type: any, fn: Function) {
    let events = this[EVENTS].get(type);
    if (events) {
...
function Event(this: any, type: any, target?: any, ) {
  this.type = type;
...
function mainThread() {
  ...
  class Worker extends EventTarget {
    constructor(url:string, options:any) {
      super();
      const {name, type, workerData} = options || {};
      url += "";
      let mod;
      if (/^data:/.test(url)) {
        mod = url;
      ...
      const worker = new threads.Worker(__filename, {
        workerData: {mod, name, type, ...workerData},
      });
